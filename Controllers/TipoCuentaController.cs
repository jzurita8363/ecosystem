...
namespace ecosystem.Controllers
{
    public class TipoCuentaController : Controller
    {
        ...
        public String ListaTipoCuenta()//crear JSON TipoVehiculo
        {
            string cadena = "{\"data\":[";
            int i = 0;
        ...
        public JsonResult ActualizarTipoCuenta(List<TipoCuenta> dataToProcess)
        {
            TipoCuenta Banco = new TipoCuenta();
            foreach (var item in dataToProcess)
            {
                ...
                {
                    con.Open();
                    MySqlCommand cmd = new MySqlCommand(bd + "web_pactualiza_tip_cuentas", con);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("p_Cod_Cuenta", Banco.Cod_Cuenta);
                    cmd.Parameters.AddWithValue("p_Nom_Cuenta", Banco.Nom_Cuenta);
                    cmd.Parameters.AddWithValue("p_User_Log", Banco.User_Log);
        ...
        public String editTipoCuenta(string Cod_Cuenta)//crear JSON Vehiculos
        {
            string cadena = "";
            string constr = conexion;
            using (MySqlConnection con = new MySqlConnection(constr))
            {
                string query = "select JSON_OBJECT('Cod_Cuenta', Cod_Cuenta, 'Nom_Cuenta', Nom_Cuenta) AS json_string from web_tab_tip_cuentas WHERE Cod_Cuenta='" + Cod_Cuenta + "'";
                using (MySqlCommand cmd = new MySqlCommand(query))
                {
                    cmd.Connection = con;
                    con.Open();
        ...
        public String EliminarTipoCuenta(string Cod_Cuenta, string User_Log) //crear JSON para eliminar y traer mensaje
        {
            string cadena = "";
            string constr = conexion;
            using (MySqlConnection con = new MySqlConnection(constr))
            {
                string query = "call web_pelimina_tip_cuentas ('" + Cod_Cuenta + "', '" + User_Log + "' )";
                using (MySqlCommand cmd = new MySqlCommand(query))
                {
                    cmd.Connection = con;
                    con.Open();
